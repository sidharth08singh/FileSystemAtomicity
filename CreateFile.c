...
void all_or_nothing_create(char filename[], int tid)
{
	...
	printf("Three Logical File Sectors are:\n%s\n%s\n%s\n", sector1, sector2, sector3);
	sprintf(create_cmd, "touch %s", sector1);
	...
	{
		sprintf(create_cmd, "touch %s", sector2);
		...
		{
			sprintf(create_cmd, "touch %s", sector3);
			...
			{
				//commit
				add_log_record_entry("create", "commit", filename, tid);
				add_log_record_entry("create", "end", filename, tid);
			}
		}
	}
}	
